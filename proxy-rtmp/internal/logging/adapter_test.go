...
func TestWithField(t *testing.T) {
	// Note: WithField is not implemented in this version, test WithFields instead
	adapter, err := NewLogrusAdapter("test")
	if err != nil {
		t.Fatalf("NewLogrusAdapter failed: %v", err)
	}

	newAdapter := adapter.WithFields(map[string]interface{}{"key": "value"})
	if newAdapter == nil {
		t.Fatal("expected non-nil adapter")
	}
	if len(newAdapter.fields) != 1 {
...
func TestChainedWithFields(t *testing.T) {
	...
	chained := adapter.
		WithFields(map[string]interface{}{"field1": "value1"}).
		WithFields(map[string]interface{}{"field2": 42}).
