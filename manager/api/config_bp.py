...
async def license_config():
    ...
    if request.method == 'GET':
        try:
            license_key = os.getenv('LICENSE_KEY', '').replace(os.getenv('LICENSE_KEY', '')[-4:], '****')
            release_mode = os.getenv('RELEASE_MODE', 'false').lower() == 'true'
        ...
        async def update_license_config(user_data):
            try:
                ...
                if 'release_mode' in data_json:
                    ...
                    logger.info(f"License mode changed to {'strict' if release_mode else 'permissive'}")
                ...
                return jsonify({
                    "error": "Failed to update license config",
                    "details": str(e)
                }), 500
...
async def logging_config():
    ...
    if request.method == 'GET':
        try:
            ...
            return jsonify({
                "error": "Failed to get logging config",
                "details": str(e)
            }), 500
...
async def features_config(user_data):
    ...
    try:
        ...
        return jsonify({
            "error": "Failed to get features config",
            "details": str(e)
        }), 500
