...
async def clusters_list():
    ...
    if request.method == 'GET':
        ...
        user = user_data.get('user') if isinstance(user_data, dict) and 'user' in user_data else user_data
    ...
    if request.method == 'GET':
        ...
        async def get_cluster_handler(user_data):
            ...
            if not user['is_admin']:
                user_role = UserClusterAssignmentModel.check_user_cluster_access(db, user['user_id'], cluster_id)
                if not user_role:
            ...
            return jsonify(ClusterResponse(
                id=cluster.id,
                name=cluster.name,
                description=cluster.description,
                syslog_endpoint=cluster.syslog_endpoint,
