...
class StructuredFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        log_entry = {
            ...
            "service": {"name": self.service_name, "version": self.version},
            "host": {"hostname": self.hostname},
            "process": {"pid": os.getpid(), "thread": threading.current_thread().name},
        }
...
class MarchProxyLogger:
    ...
    def log_license_event(
        ...
        logger.log(
            level,
            f"License {event_type}: {'success' if success else 'failed'}",
            extra=extra,
        )
