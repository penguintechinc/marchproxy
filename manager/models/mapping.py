...
class MappingModel:
    ...
    @staticmethod
    def _normalize_service_list(
        db: DAL, services: List[Union[int, str]], cluster_id: int
    ) -> List[Dict[str, Any]]:
    ...
    def _normalize_port_list(ports: List[Union[int, str]]) -> List[Dict[str, Any]]:
        ...
        for port in ports:
            if isinstance(port, int):
                # Single port
                if 1 <= port <= 65535:
                    normalized.append({"type": "single", "port": port})
                ...
                if "-" in port and "," not in port:
                    ...
                    try:
                        ...
                        if valid_ports:
                            normalized.append({"type": "list", "ports": valid_ports})
                    except ValueError:
                        pass
                    ...
                    try:
                        port_num = int(port)
                        if 1 <= port_num <= 65535:
                            normalized.append({"type": "single", "port": port_num})
                    except ValueError:
                        pass
            ...
            if not user or not user.get("is_admin", False):
                ...
                for mapping in all_mappings:
                    ...
                    for source in mapping.source_services:
                        if (
                            source.get("type") == "service"
                            and source.get("id") in accessible_service_ids
                        ):
                            has_access = True
                            break
                    ...
                    if not has_access:
                        for dest in mapping.dest_services:
                            if (
                                dest.get("type") == "service"
                                and dest.get("id") in accessible_service_ids
                            ):
                                has_access = True
                                break
    ...
    def resolve_mapping_services(db: DAL, mapping_id: int) -> Optional[Dict[str, Any]]:
        ...
        for source in mapping.source_services:
            resolved_sources.extend(
                MappingModel._resolve_service_reference(db, source, mapping.cluster_id)
            )

        # Resolve destination services
        resolved_destinations = []
        for dest in mapping.dest_services:
            resolved_destinations.extend(
                MappingModel._resolve_service_reference(db, dest, mapping.cluster_id)
            )
    ...
    def _resolve_service_reference(
        db: DAL, service_ref: Dict[str, Any], cluster_id: int
    ) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def find_matching_mappings(
        db: DAL, source_service_id: int, dest_service_id: int, protocol: str, port: int
    ) -> List[Dict[str, Any]]:
        ...
        for mapping in mappings:
            if (
                MappingModel._service_matches(
                    db, source_service_id, mapping.source_services, cluster_id
                )
                and MappingModel._service_matches(
                    db, dest_service_id, mapping.dest_services, cluster_id
                )
                and protocol in mapping.protocols
                and MappingModel._port_matches(port, mapping.ports)
            ):
                matching.append(
                    {
                        "id": mapping.id,
    ...
    def _service_matches(
        db: DAL, service_id: int, service_refs: List[Dict[str, Any]], cluster_id: int
    ) -> bool:
...
class CreateMappingRequest(BaseModel):
    ...
    def validate_name(cls, v):
        ...
        if not v.replace("-", "").replace("_", "").isalnum():
            raise ValueError(
                "Mapping name can only contain alphanumeric characters, hyphens, and underscores"
            )
        return v.lower()
