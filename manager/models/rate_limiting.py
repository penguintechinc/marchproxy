...
class RateLimitManager:
    ...
    def check_limit(
        self, client_id: str, endpoint: str, endpoint_type: str = "api_general"
    ) -> Tuple[bool, Dict[str, Any]]:
    ...
    def get_client_identifier(self, request, user=None) -> str:
        ...
        remote_addr = request.headers.get("X-Real-IP") or request.environ.get(
            "REMOTE_ADDR", "unknown"
        )
...
class XDPRateLimitModel:
    ...
    def define_ip_whitelist_table(db: DAL):
        ...
        return db.define_table(
            ...
            Field("description", type="text"),
            Field(
                "whitelist_type", type="string", length=32, default="manual"
    ...
    def create_default_config(db: DAL, cluster_id: int, user_id: int) -> Optional[int]:
        ...
        try:
            ...
            logger.info(
                f"Created default XDP rate limiting config {rate_limit_id} for cluster {cluster_id}"
            )
    ...
    @staticmethod
    def get_proxy_stats(
        db: DAL, rate_limit_id: int, proxy_id: int, hours: int = 24
    ) -> Dict[str, Any]:
...
class XDPRateLimitManager:
    ...
    def create_rate_limit(
        self, cluster_id: int, config: Dict[str, Any], user_id: int
    ) -> Tuple[bool, Any]:
    ...
    def update_rate_limit(
        self, rate_limit_id: int, config: Dict[str, Any], user_id: int
    ) -> Tuple[bool, Any]:
    ...
    def get_proxy_config(self, cluster_id: int, proxy_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "enterprise_enabled": (
                self.license_manager.has_feature("xdp_rate_limiting")
