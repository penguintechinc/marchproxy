...
class ServiceModel:
    ...
    @staticmethod
    def set_jwt_auth(
        db: DAL, service_id: int, expiry_seconds: int = 3600, algorithm: str = "HS256"
    ) -> Optional[str]:
    ...
    def rotate_jwt_secret(db: DAL, service_id: int) -> Optional[str]:
        ...
        service.update_record(jwt_secret=new_secret, updated_at=datetime.utcnow())
...
class UserServiceAssignmentModel:
    ...
    def assign_user_to_service(db: DAL, user_id: int, service_id: int, assigned_by: int) -> bool:
        ...
        db.user_service_assignments.insert(
            user_id=user_id, service_id=service_id, assigned_by=assigned_by
        )
        return True
