    ...
    def check_keepalive_health(db: DAL, license_key: str) -> Dict[str, Any]:
        ...
        if not cache_entry or not cache_entry.is_enterprise:
            return {
                ...
                "message": "Community edition or no license",
            }
...
class LicenseValidator:
    ...
    async def validate_license(
        self, db: DAL, license_key: str, force_refresh: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if validation_result.get("expires_at"):
                expires_at = datetime.fromisoformat(
                    validation_result["expires_at"].replace("Z", "+00:00")
                )
    ...
    async def _call_license_server(self, license_key: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            ...
            if response.status_code == 200:
                ...
                return {
                    "valid": False,
                    "error": "License not found, inactive, or expired",
                }
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    ...
                    json={"product": "marchproxy", "feature": feature},
                    headers={
