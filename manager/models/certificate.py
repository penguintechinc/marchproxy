...
class CertificateModel:
    ...
    @staticmethod
    def create_certificate(db: DAL, name: str, cert_data: str, key_data: str,
                          source_type: str, created_by: int, description: str = None,
                          ca_bundle: str = None, source_config: Dict = None,
                          auto_renew: bool = False, renewal_threshold_days: int = 30) -> int:
    ...
    @staticmethod
    def update_renewal_attempt(db: DAL, cert_id: int, success: bool,
                              new_cert_data: str = None, new_key_data: str = None,
        ...
        if success and new_cert_data and new_key_data:
            ...
            if cert_info:
                ...
                update_data['next_renewal_check'] = max(next_check, datetime.utcnow() + timedelta(days=1))
...
class InfisicalCertificateProvider:
    ...
    async def fetch_certificate(self, secret_path: str, environment: str = "prod") -> Optional[Dict[str, str]]:
...
class VaultCertificateProvider:
    ...
    async def issue_certificate(self, role: str, common_name: str,
                               alt_names: List[str] = None, ttl: str = "720h") -> Optional[Dict[str, str]]:
...
class CertificateManager:
    ...
    def create_from_infisical(self, name: str, infisical_config: Dict[str, str],
                             created_by: int, auto_renew: bool = True) -> Optional[int]:
        ...
        return CertificateModel.create_certificate(
            self.db, name, "", "",  # cert_data and key_data will be fetched
    ...
    def create_from_vault(self, name: str, vault_config: Dict[str, str],
                         created_by: int, auto_renew: bool = True) -> Optional[int]:
        ...
        return CertificateModel.create_certificate(
            self.db, name, "", "",  # cert_data and key_data will be issued
            source_type='vault',
            created_by=created_by,
    ...
    def create_from_upload(self, name: str, cert_data: str, key_data: str,
                          created_by: int, ca_bundle: str = None) -> int:
        """Create certificate from direct upload"""
        return CertificateModel.create_certificate(
            self.db, name, cert_data, key_data,
            source_type='upload',
            created_by=created_by,
            ca_bundle=ca_bundle,
            auto_renew=False
...
class TLSProxyCAModel:
    ...
    def define_table(db: DAL):
        ...
        return db.define_table(
            ...
            Field('wildcard_domain', type='string', required=True, length=255),  # e.g., *.company.com
    ...
    def define_proxy_config_table(db: DAL):
        ...
        return db.define_table(
            ...
            Field('enabled', type='boolean', default=False),
            Field('protocol_detection', type='boolean', default=True),  # Use protocol fingerprinting
            Field('port_based_detection', type='boolean', default=False),  # Fallback to port detection
            Field('target_ports', type='json'),  # List of ports to monitor (if port-based)

            # TLS Proxy Behavior
            Field('intercept_mode', type='string', default='transparent', length=20),  # transparent, explicit
            Field('certificate_validation', type='string', default='none', length=20),  # none, warn, strict
            Field('preserve_sni', type='boolean', default=True),
            Field('log_connections', type='boolean', default=True),
            Field('log_decrypted_content', type='boolean', default=False),  # SECURITY: disabled by default
    ...
    @staticmethod
    def generate_self_signed_ca(domain: str, key_type: str = 'ecc',
                               key_size: int = 384, hash_algorithm: str = 'sha512',
                               lifetime_years: int = 10) -> Dict[str, str]:
        ...
        try:
            ...
            wildcard_cert = x509.CertificateBuilder().subject_name(
                ...
                wildcard_private_key.public_key()
            ).serial_number(
                x509.random_serial_number()
            ).not_valid_before(
                datetime.utcnow()
            ...
            wildcard_cert_pem = wildcard_cert.public_bytes(serialization.Encoding.PEM).decode('utf-8')
            ...
            return {
                ...
                'ca_fingerprint': hashlib.sha256(ca_cert.public_bytes(serialization.Encoding.DER)).hexdigest(),
    ...
    @staticmethod
    def create_tls_proxy_ca(db: DAL, cluster_id: int, name: str, domain: str,
                           user_id: int, ca_data: Dict[str, str] = None,
...
class TLSProxyConfigManager:
    ...
    def create_tls_proxy_config(self, cluster_id: int, ca_id: int,
                               config: Dict[str, Any], user_id: int) -> Tuple[bool, Any]:
    ...
    def get_proxy_config(self, cluster_id: int, proxy_id: int) -> Dict[str, Any]:
        ...
        if not config:
            return {
                'enabled': False,
                'enterprise_available': self.license_manager.has_feature('tls_proxy') if self.license_manager else False
            }
