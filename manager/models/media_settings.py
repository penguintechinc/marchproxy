...
class MediaSettingsModel:
    ...
    def get_settings(db: DAL) -> Optional[dict]:
        ...
        return {
            ...
            'transcode_ladder_resolutions': settings.transcode_ladder_resolutions or [360, 540, 720, 1080],
            'updated_by': settings.updated_by,
            'updated_at': settings.updated_at,
        }

    @staticmethod
    def update_settings(db: DAL, updated_by: int,
                       ...
                       transcode_ladder_resolutions: Optional[List[int]] = None) -> dict:
        ...
        if settings:
            ...
            if admin_max_resolution is not None:
                update_data['admin_max_resolution'] = admin_max_resolution if admin_max_resolution > 0 else None
            if admin_max_bitrate_kbps is not None:
                update_data['admin_max_bitrate_kbps'] = admin_max_bitrate_kbps if admin_max_bitrate_kbps > 0 else None
            ...
            db.media_settings.insert(
                admin_max_resolution=admin_max_resolution if admin_max_resolution and admin_max_resolution > 0 else None,
                admin_max_bitrate_kbps=admin_max_bitrate_kbps if admin_max_bitrate_kbps and admin_max_bitrate_kbps > 0 else None,
                enforce_codec=enforce_codec if enforce_codec and enforce_codec != '' else None,
                transcode_ladder_enabled=transcode_ladder_enabled if transcode_ladder_enabled is not None else True,
...
class MediaStreamModel:
    ...
    @staticmethod
    def create_stream(db: DAL, stream_key: str, protocol: str, client_ip: str,
                     codec: str = None, resolution: str = None,
                     bitrate_kbps: int = None, user_agent: str = None) -> int:
