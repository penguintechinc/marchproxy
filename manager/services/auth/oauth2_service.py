...
class OAuth2Service:
    ...
    def handle_oauth2_callback(self, provider: str, code: str, state: str) -> Dict:
        ...
        try:
            ...
            for key in [
                ...
                f"oauth2_redirect_{provider}",
            ]:
                session.pop(key, None)

            logger.info(
                f"OAuth2 authentication successful for provider {provider}, user: {user['email']}"
            )
            return user
    ...
    def _provision_oauth2_user(
        self, provider: str, user_attributes: Dict, token_data: Dict
    ) -> Dict:
