...
class ServiceManagementService:
    ...
    def assign_user_to_service(
        self, service_id: int, target_user_id: int, role: str, assigner_user_id: int
    ) -> bool:
    ...
    def remove_user_from_service(
        self, service_id: int, target_user_id: int, remover_user_id: int
    ) -> bool:
    ...
    def regenerate_token(self, service_id: int, user_id: int) -> Dict:
        ...
        self.db(self.db.services.id == service_id).update(
            token_base64=new_token, updated_at=datetime.utcnow(), updated_by=user_id
        )
        self.db.commit()

        logger.info(f"Regenerated token for service {service_id}")

        return {"token": new_token, "message": "Token regenerated successfully"}
    ...
    def get_service_auth_config(self, service_id: int) -> Dict:
        ...
        auth_config = {"service_id": service_id, "auth_type": service.auth_type}
        ...
        if not user.is_admin:
            query &= self.db.services.id.belongs(
                self.db(self.db.user_service_assignments.user_id == user_id)._select(
                    self.db.user_service_assignments.service_id
                )
            )
