...
class KillKrillService:
    ...
    def send_log(self, level: str, message: str, **kwargs):
        ...
        for key, value in kwargs.items():
            if key not in ['hostname', 'timestamp', 'tags', 'trace_id', 'span_id', 'transaction_id']:
                labels[key] = value
    ...
    def send_metric(self, name: str, metric_type: str, value: float, labels: Optional[Dict[str, str]] = None, help_text: Optional[str] = None):
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        # Check log endpoint
        try:
            response = self.session.get(f"{self.log_endpoint.replace('/api/v1/logs', '/healthz')}", timeout=5)
        ...
        try:
            response = self.session.get(f"{self.metrics_endpoint.replace('/api/v1/metrics', '/healthz')}", timeout=5)
